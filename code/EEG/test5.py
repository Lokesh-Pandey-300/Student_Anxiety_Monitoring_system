...
if not X_all.empty:
    ...
    classifiers = {
        ...
        'VC(RF+DT)': voting_classifier1,
        'VC(SVC+DT)': voting_classifier2,
        'VC(SVC+RF)': voting_classifier3,
        'VC(SVC+RF+DT)': voting_classifier4,
        'VC(SVC+RF+DT+XGb)': voting_classifier5,
    }
...
# Plotting bars
precision_bars = ax.bar(classifiers.keys(), precision_list, width=bar_width, label='Precision')
recall_bars = ax.bar([x + bar_width for x in classifiers.keys()], recall_list, width=bar_width, label='Recall')
f1_bars = ax.bar([x + 2 * bar_width for x in classifiers.keys()], f1_list, width=bar_width, label='F1 Score', alpha=0.7)

# Adding labels and title
plt.xlabel('Classifiers', fontsize=14)  # Increase font size for better visibility
plt.ylabel('Metrics', fontsize=14)  # Increase font size for better visibility
plt.title('Precision, Recall, and F1 Score Over Classifiers', fontsize=16)  # Increase font size for better visibility
plt.legend()
...
plt.xlabel('Classifiers', fontsize=14)  # Increase font size for better visibility
plt.ylabel('Accuracy', fontsize=14)  # Increase font size for better visibility
plt.title('Training and Testing Accuracy Over Classifiers', fontsize=16, fontweight='bold')  # Increase font size for better visibility and make title bold
plt.legend()

# Rotating classifier names for better readability
plt.xticks(rotation=45, ha='right', fontsize=12)  # Increase font size for better visibility

# Add names of classifiers on the bars (bold and straight)
for bar, classifier_name in zip(train_bars, classifiers.keys()):
    yval = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2, yval + 0.01, classifier_name, ha='center', va='bottom', rotation=0, fontweight='bold')  # Set rotation to 0 and make text bold
...
# Add names of classifiers on the bars
for bar, classifier_name in zip(precision_bars, classifiers.keys()):
    yval = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2, yval + 0.01, classifier_name, ha='center', va='bottom', rotation=45)
