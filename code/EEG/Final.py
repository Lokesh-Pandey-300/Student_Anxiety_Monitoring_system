...
if not X_all.empty:
    ...
    print(f"\nAccuracy for all Classes of Anxiety: {accuracy}")
    ...
    # Plot data distribution column chart
    #label_counts = y_all.value_counts()
    #labels = ['Normal', 'Light Anxiety', 'Moderate Anxiety', 'Severe Anxiety']
    #participants_counts = [label_counts.get(label, 0) for label in range(1, 5)]

    # Normalize the counts to be proportional to the total number of participants
    #participants_percentages = [count / total_participants * 100 for count in participants_counts]

    #plt.bar(labels, participants_percentages, color=['blue', 'green', 'orange', 'red'])
    #plt.xlabel('Emotion Levels')
    #plt.ylabel('Percentage of Participants')
    #plt.title('Data Distribution of Emotion Levels')
    #plt.show()
